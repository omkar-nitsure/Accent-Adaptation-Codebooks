...
resume=/raid/speech/omkar/Accented_ASR/finetuning/espnet-master/egs/commonvoice/asr1/exp/final_exps/ctc_ce/snapshot.ep.100       # Resume the training from snapshot
# resume=
...
csvdir=/raid/speech/omkar/Accented_ASR/finetuning/accent_splits/csvs     # Path to the directory hosting all our csvs. 
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    ${cuda_cmd} --gpu ${ngpu} ${expdir}/train.log \
        ...
        --verbose ${verbose} \
        --resume ${resume} \
        --train-json ${feat_tr_dir}/data_${bpemode}${nbpe}_with_accent_thresh_1_2.json \
        --valid-json ${feat_dt_dir}/data_${bpemode}${nbpe}_with_accent_thresh_1_2.json \
        --freeze-mods [""]
fi
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    for rtask in ${recog_set}; do
    (
        decode_dir=decode_test_${rtask}_$(basename ${decode_config%.*})_${lmtag}
        feat_recog_dir=${dumpdir}/${rtask}/delta${do_delta}
