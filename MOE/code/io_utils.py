...
class LoadInputsAndTargets(object):
    ...
    def __call__(self, batch, return_uttid=False):
        ...
        :return: list of ideal codebook weights
        ...
        y_probs_ideal_dict = OrderedDict() 
        uttid_list = []  # List[str]
        ...
        for uttid, info in batch:
            ...
            if self.load_output:
                ...
                x = np.array(info["probs"])
                y_probs_ideal_dict.setdefault("label-1", []).append(x)
            # x = np.array(info["accent"])
            # y_acc_labels_dict.setdefault("label1", []).append(x)
        ...
        if self.mode == "asr":
            return_batch, uttid_list = self._create_batch_asr(
                x_feats_dict, y_feats_dict, y_acc_labels_dict, y_probs_ideal_dict, uttid_list
            )
        elif self.mode == "tts":
            _, info = batch[0]
    ...
    def _create_batch_asr(self, x_feats_dict, y_feats_dict,y_acc_labels_dict, y_ideal_probs, uttid_list):
        ...
        if self.load_output:
            ...
            y_probs = list(y_ideal_probs.values())
        ...
        if self.load_output:
            ...
            y_probs = [[yp[i] for i in nonzero_sorted_idx] for yp in y_probs]
            y_prob_names = list(y_ideal_probs.keys())
            ...
            return_batch = OrderedDict(
                [
                    ...
                    *[(y_prob_name, y_prob) for y_prob_name, y_prob in zip(y_prob_names, y_probs)],
                ]
            )
        else:
            return_batch = OrderedDict([
