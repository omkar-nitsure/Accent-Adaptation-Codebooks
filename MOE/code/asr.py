...
class CustomConverter(object):
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        xs, ys, acc_label, prob_true = batch[0]
        # xs,ys = batch[0]
        ...
        probs_true = torch.from_numpy(numpy.asarray(prob_true)).to(device)

        # return xs_pad,ilens,ys_pad
        return xs_pad, ilens, ys_pad, acc_labels, probs_true
...
def train(args):
    ...
    # Wandb plotting ##
    wandb.login(key="21afa1177d65c8ed927a0bbb779f9ac889087b0e")
    run = wandb.init(
        project="codebook_gating"
    )
    ...
    if args.resume:
        ...
        torch_resume(args.resume, trainer, model)
        ...
        with torch.no_grad():
            for idx, name in enumerate(js.keys(), 1):
                ...
                if args.streaming_mode == "window" and args.num_encs == 1:
                    ...
                    nbest_hyps, beam_accs = model.recognize(feat, beam_search_transducer, is_train=False)
                else:
                    nbest_hyps, beam_accs = model.recognize(
                        feat, args, train_args.char_list, rnnlm, is_train=False
                    )
                new_js[name] = add_results_to_json(
                    js[name], nbest_hyps, train_args.char_list
                )
                new_js[name].update({"beams": beam_accs})
