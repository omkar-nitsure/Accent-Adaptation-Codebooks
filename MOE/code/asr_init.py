    ...
    for i in range(len(m_keys)):
        ...
        if(n[0] == "encoder"):
            for j in range(1, len(n)):
                if(n[j] == "codebook_embedding"):
                # if((n[j] == "gating_linear") or ((n[j] == "codebook_attention") and ((n[j + 1] == "linear_k") or (n[j + 1] == "linear_q")))):
                # if(n[j] == "gating_linear"):
                # if((n[j] == "gating_linear") or (n[j] == "codebook_embedding") or (n[j] == "codebook_attention") or (n[j] == "0") or (n[j] == "1") or (n[j] == "2")):
                # if((n[j] == "gating_linear") or (n[j] == "codebook_embedding") or (n[j] == "codebook_attention")):
                # if((n[j] == "gating_linear") or (n[j] == "embed") or ((n[j] == "codebook_attention") and ((n[j + 1] == "linear_k") or (n[j + 1] == "linear_v") or (n[j + 1] == "linear_out"))) or ((n[j] == "0") and ((n[j + 1] == "self_attn") or (n[j + 1] == "feed_forward_macaron") or (n[j + 1] == "norm_mha") or (n[j + 1] == "norm_ff_macaron")))):
                    fl.append(m_keys[i])
                    break

    # fl.append("encoder.codebook_embedding")
    # # not_freeze_list = ["encoder.gating_linear.0.weight", "encoder.gating_linear.0.bias", "encoder.gating_linear.2.weight", "encoder.gating_linear.2.bias"]
    # # not_freeze_list = ["encoder.codebook_embedding", "encoder.gating_linear.0.weight", "encoder.gating_linear.0.bias", "encoder.gating_linear.2.weight", "encoder.gating_linear.2.bias"]


    # # for i in range(len(not_freeze_list)):
    # #     nfl.append(not_freeze_list[i])
    # # Changes end here

    for mod, param in model.named_parameters():

        # if not any(mod.startswith(m) for m in nfl):
        if any(mod.startswith(m) for m in fl):
            logging.warning(f"Freezing {mod}. It will not be updated during training.")
            param.requires_grad = False
        else:
            logging.warning(f"Training {mod}. It will be updated during training.")
