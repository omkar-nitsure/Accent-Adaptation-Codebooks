...
class HubertCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True, log_pred=False):
        ...
        budget_loss_list = []
        for l in range(len(net_output["attention_weights"])):
            budget_loss = torch.mean(1 - torch.sum(net_output["attention_weights"][l], axis=-1))
            budget_loss_list.append(budget_loss)
            logging_output["budget_loss"] = budget_loss.detach().item()
        
        loss += sum(budget_loss_list)
