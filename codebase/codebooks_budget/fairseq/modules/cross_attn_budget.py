...
def cross_attn_fn(
    ...
    if need_weights:
        ...
        # if(is_cross_attn==True):

        #     #attn_vals = attn_output_weights
        #     mean = torch.mean(attn_output_weights, axis=-1)
        #     std = torch.std(attn_output_weights, axis=-1)

        #     thresh = mean + std
        #     thresh = torch.unsqueeze(thresh, axis=-1)

        #     zero_idx = attn_output_weights < thresh
        #     mask = torch.ones_like(attn_output_weights)
        #     mask[zero_idx] = 0
        ...
        if(is_cross_attn==True):
            ...
            _, indices = torch.topk(attn_output_weights, k=50, dim=-1)
            mask = torch.zeros_like(attn_output_weights)

            mask.scatter_(-1, indices, value = 1)
