...
class HubertModel(BaseFairseqModel):
    def __init__(
        ...
        if any([d is None for d in dictionaries]):
            ...
            self.label_embs_concat = nn.Parameter(
                torch.FloatTensor(524, final_dim)
            )
    ...
    def forward(
        ...
        # splitting the centers into different accents

        # TODO: take a parameter from the earlier file and compute the clusters for accent here
        label_embs_list = self.label_embs_concat.split([254, 270], 0)
        ...
        if not self.skip_masked:
            ...
            logit_m_list = [compute_pred(proj_x_m_list[i], target_list[i][masked_indices], label_embs_list[i])
        ...
        if not self.skip_nomask:
            ...
            logit_u_list = [compute_pred(proj_x_u_list[i], target_list[i][nomask_indices], label_embs_list[i])
                            for i in range(len(target_list))]
